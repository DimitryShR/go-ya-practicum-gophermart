...
- уровень логирования: переменная окружения ОС `LOG_LEVEL` или флаг `--log-level` (по умолчанию `info`)

- путь к файлу для записи логов: переменная окружения ОС `LOG_FILENAME` или флаг `--log-file`

- секретный ключ для подписи JWT токенов: переменная окружения ОС `JWT_SECRET` или флаг `-j` (по умолчанию `gophermart-dev-secret`)

- время жизни JWT токена: переменная окружения ОС `JWT_TTL` или флаг `--jwt-ttl` (по умолчанию `72h`)

- интервал опроса системы расчёта начислений: переменная окружения ОС `ACCRUAL_POLL_INTERVAL` или флаг `--poll-interval` (по умолчанию `2s`)

- таймаут корректного завершения работы сервиса: переменная окружения ОС `SHUTDOWN_TIMEOUT` или флаг `--shutdown-timeout` (по умолчанию `10s`)
